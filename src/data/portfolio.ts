...
export const projects: Project[] = [
  {
    ...
    features: [
      { title:  '인수인계 기반 문서 체계 수립',
        detail: '사업 인수 초기 단계로 인수인계 산출물 정리, 시스템 아키텍처 구성도 작성, 운영 매뉴얼 작성을 통해 안정적인 유지보수 기반 마련'
      },
    ],
  },
  {
    ...
    features: [
      { title:  'Google Maps API 클러스터링으로 상담 이력 발신 국가 시각화',
        detail: '상담 이력 발신 국가 최근 한달 데이터를 조회하여 Google Maps API 클러스터링으로 국가별 건수를 지도에 시각화'
      },
      { title:  '상담 팀장 이상 권한 상담 이력 및 피드백 조회 페이지 개발',
        detail: '상담 이력 및 피드백 항목(정보 전달 미숙, 상담 유형 오선택 등)을 세분화하여 팀장 이상 권한에서만 접근 가능한 조회 페이지 신규 개발'
      },
      { title:  '조치 이력 개인/공용 템플릿 저장 및 불러오기 기능 개발',
        detail: '팀장 이상 권한은 공용 템플릿으로 저장하여 전체 사용자가 활용 가능하고, 일반 상담원은 개인 템플릿으로만 저장되어 본인에게만 노출되도록 권한별 분기 구현'
      },
      ...
      { title:  '해외로밍 안전문자 작업 이력 비교 조회 기능 개발',
        detail: '기존에는 단일 이력 row를 계속 덮어 이전 내용과 비교가 불가능했던 구조를 스냅샷 방식으로 개선하여 이전 이력 보존 및 변경 전후 비교 조회 기능 구현'
      },
      { title:  '문자 발송 이력 페이지 쿼리 성능 개선',
        detail: '제니퍼 APM 응답 지연 감지를 확인하여 WHERE절 TO_CHAR 함수 제거, 조인 필드 인덱스 추가로 응답 시간 3초 → 1.5초로 단축'
      },
      { title:  '해외로밍 안전문자 발송 메시지 유니코드 정규식 검증 추가',
        detail: '통신 3사 수신 환경에서 2바이트 공백 및 특수문자가 "?"로 표시되거나 내용이 잘리는 문제를 정규식 검증으로 입력 단계에서 사전 차단'
      },
    ],
  },
];
